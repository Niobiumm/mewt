...
async def on_ready():
    ...
    print("JavaAutomation is now running in background! Do !info to view the commands!")
            ...
            for proc in psutil.process_iter():
                ...
                if name == "python.exe":
                    ...
                    if "main.py" in cmdline[1]:
                        pid = proc.pid
                        os.kill(pid, signal.SIGTERM)
            subprocess.Popen(["python", 'ExtenderRunner.py'])
            ...
            for proc in psutil.process_iter():
                ...
                if name == "python.exe":
                    ...
                    if "main.py" in cmdline[1]:
                        pid = proc.pid
                        os.kill(pid, signal.SIGTERM)
            subprocess.Popen(["python", 'JavaAutomation.py'])
            __builtins__.print("Finished Running, ending launcher, mewt sniper and this script...")
            exit()
    else:
